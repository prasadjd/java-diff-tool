 ...
 */
public class JavaClassDiffGeneratorHTML {


    private static Set<String> claasToExclude = new HashSet<>();
    static {
        claasToExclude.addAll(Arrays.asList("{classNamesToBeIgnoredInDiffCheck}"));
    }
    ...
    public static void main(String s[]) throws Exception {
        ...
        try {
            File finalFile = new File("{outPutFilePath}");
    ...
    private static void process(File leftDirFile, File rightDirFile, BufferedWriter fileWriter) throws Exception {
        ...
        if (leftDirFile.isDirectory()) {
            ...
            if (!(leftDirFile.getAbsolutePath().toLowerCase(Locale.ROOT).contains("{sourceDirectory}") || leftDirFile.getAbsolutePath().toLowerCase(Locale.ROOT).contains("{targetDirectory}")) || leftDirFile.getName().toLowerCase(Locale.ROOT).contains("test")) {
                return;
            }
            ...
            for (File rightFile : finalFiles){
                ...
                if (headers.contains("DIFF_FILE")) {
                    List<String> changedMethods = JavaClassDiffGeneratorHTML.methodDiffInClass(
    ...
    public static List<String> methodDiffInClass(String file1, String file2, BufferedWriter fileWriter) throws Exception {
        List<String> changedMethods = new ArrayList<>();
        HashMap<String, String> methods = new HashMap<>();

        JavaClassDiffGeneratorHTML md = new JavaClassDiffGeneratorHTML();
    ...
    private static final Formatter formatter = new Formatter(JavaFormatterOptions.defaultOptions());
    private static String getRow(String methodSignature, String body1, String body2) throws Exception {
    ...
    private static String getDifference(String body1, String body2) {//.append(";</span></br>")
        List<DiffVO> diffV2 = getDiffV2(body1, body2);
    ...
    private static List<DiffVO> getDiffV2(String body1, String body2) {
        ...
        for(int i=0;(i<=splitStr1.length-1 || i<=splitStr2.length-1);i++){
            ...
            if(!s1.equalsIgnoreCase(s2) && s1.trim().length()!=0){
                ...
                if(exist){
                    diffVOS.add(new DiffVO("<span style=\"color:#"+convertStringToHex(s1)+"\">",s1));
                }else {

                    diffVOS.add(new DiffVO("<span style=\"color:#003380\">", s1));
                }
            }
        }
